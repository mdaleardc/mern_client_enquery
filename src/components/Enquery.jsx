...
const Enquery = () => {
  ...
  // get all enquery list
    ...
    try {
      const response = await axios(`${api_url}/view`);
      setEnqueryList(response.data);
    } catch (err) {
      console.error("Something went wrong to view enquery data", err.message);
    }}
  ...
  const handleSubmit = async (e) => {
    ...
    if(formData._id) {
      try {
        const response = await axios.put(`${api_url}/update/${formData._id}`, formData);
  ...
  const deleteRow = async (delId) => {
    ...
    try {
    if(isConfirm) {
      const response = await axios.delete(`${api_url}/delete/${delId}`);
  ...
  const editRow = async (editId) => {
    try {
      const response = await axios(`${api_url}/single-row/${editId}`);
